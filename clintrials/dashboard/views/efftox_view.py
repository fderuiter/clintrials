...
def render(sims):
    ...
    func_map = {
        'N': lambda s, p: len(s),
        'recommended_dose_prob': lambda s, p: pd.Series([x.get('recommended_dose') for x in s]).value_counts(normalize=True).sort_index(),
        'prob_accept_tox': lambda s, p: pd.Series([x.get('prob_accept_tox', 0) > 0.5 for x in s]).mean(),
        'prob_accept_eff': lambda s, p: pd.Series([x.get('prob_accept_eff', 0) > 0.5 for x in s]).mean()
    }
    ...
    try:
        ...
        if not summary_df.empty:
            ...
            if 'prob_accept_tox' in summary_df.columns and 'prob_accept_eff' in summary_df.columns:
