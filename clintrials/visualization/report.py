    ...
    def _render_table_cell(  # type: ignore
        ...
        **kwargs,
        ...
        if height_query_only:
            return super()._render_table_cell(
                i, j, cell, row_height, cell_height_info, cell_x_positions, **kwargs
            )
    ...
    for obj_id, elem in elems.items():
        ...
        if elem["type"] in ("TH", "TD"):
            ...
            if parent and parent["type"] != "TR":
                raise ValueError(
                    f"{elem['type']} (id {obj_id}) must be a child of a TR element"
                )
        ...
        if elem["type"] in ("Table", "TR", "THead", "TBody", "TFoot"):
            for kid in elem["kids"]:
                if isinstance(kid, str) and kid.startswith("MCID_"):
                    raise ValueError(
                        f"Structural container {elem['type']} (id {obj_id}) cannot have MCID assigned directly"
                    )
