...
from clintrials.dosefinding.efftox import EffTox, LpNormCurve, efftox_get_posterior_probs
...
def test_adaptive_integration_extreme_case(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        probs, pds = efftox_get_posterior_probs(
            cases, priors, scaled_doses, tox_cutoff, eff_cutoff,
            n=10**4, max_iter=1, mass_threshold=0.999999
        )
    ...
    probs2, pds2 = efftox_get_posterior_probs(
        cases, priors, scaled_doses, tox_cutoff, eff_cutoff,
        n=10**4, max_iter=5, mass_threshold=0.999999
    )
    ...
    probs_exp, pds_exp = efftox_get_posterior_probs(
        cases_extreme, priors_extreme, scaled_doses, tox_cutoff, eff_cutoff,
        n=10**4, max_iter=5, mass_threshold=0.9999
    )
...
def test_efftox_class_propagation():
    ...
    trial = EffTox(
        real_doses, priors, 0.3, 0.5, 0.9, 0.9, metric, 30,
        k_sd=8.0, max_iter=5, mass_threshold=0.9999999
    )
...
def test_boundary_mass_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        efftox_get_posterior_probs(
            cases, priors, scaled_doses, 0.3, 0.5,
            n=1000, max_iter=1, mass_threshold=1.0 # threshold of 1.0 is impossible
        )
