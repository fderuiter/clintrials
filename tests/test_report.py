...
def test_pdf_structural_nesting_and_mcid():
    ...
    for tr in trs:
        ...
        assert any(str(x).startswith("MCID_") for x in tr["kids"]) is False, (
            "TR should not have MCIDs"
        )
    ...
    for cell in ths + tds:
        ...
        assert len(cell["kids"]) == 1 and str(cell["kids"][0]).startswith("MCID_"), (
            "Cells should have MCIDs"
        )
