...
def test_matchpoint_efftox():
    ...
    et = (
        EffToxBuilder()
        .with_real_doses(mp_real_doses)
        .with_theta_priors(mp_efftox_priors)
        .with_cutoffs(mp_tox_cutoff, mp_eff_cutoff)
        .with_certainties(mp_tox_certainty, mp_eff_certainty)
        .with_metric(mp_metric)
        .with_max_size(mp_trial_size)
        .with_first_dose(mp_first_dose)
        .build()
    )
    epsilon1 = 0.05
    epsilon2 = 0.05
...
class TestLpNormCurve:
    ...
    def test_solve_consistency(self, lp_norm_curve, prob_eff):
        try:
            ...
            # Infeasible delta is expected for some prob_eff
            pass
...
def efftox_trial():
    return (
        EffToxBuilder()
        .with_real_doses([1, 2, 4, 6.6, 10])
        .with_theta_priors([norm()] * 6)
        .with_cutoffs(0.3, 0.5)
        .with_certainties(0.9, 0.85)
        .with_metric(LpNormCurve(0.5, 0.3, 0.7, 0.25))
        .with_max_size(30)
        .build()
    )
...
class TestInverseQuadraticCurve:
    ...
    def test_solve(self):
        ...
        # Test delta != 0
        prob_tox = curve.solve(0.1, prob_eff=0.5)
        assert np.isclose(curve(0.5, prob_tox), 0.1, atol=1e-5)

        # Test infeasible delta
        with pytest.raises(ValueError, match="infeasible"):
            curve.solve(2.0, prob_eff=0.5)

        # Test validation
        with pytest.raises(ValueError):
            curve.solve(0)
        with pytest.raises(ValueError):
            curve.solve(0, prob_eff=0.5, prob_tox=0.3)
...
class TestEffToxAdmissibleSet:
    def test_epsilon_parameter(self):
        ...
        trial1 = (
            EffToxBuilder()
            .with_real_doses(real_doses)
            .with_theta_priors(theta_priors)
            .with_cutoffs(tox_cutoff, eff_cutoff)
            .with_certainties(tox_certainty, eff_certainty)
            .with_metric(metric)
            .with_max_size(30)
            .with_first_dose(3)
            .with_kwargs(num_integral_steps=10**3, epsilon=1e-6)
            .build()
        )
        trial2 = (
            EffToxBuilder()
            ...
            .with_max_size(30)
            .with_first_dose(3)
            .with_kwargs(num_integral_steps=10**3, epsilon=1e-2)
            .build()
        )
...
def test_myeloma_integration_deterministic(mocker):
    ...
    trial = (
        ...
        .with_cutoffs(0.3, 0.2)
        .with_certainties(0.9, 0.8)
        .with_metric(metric)
        .with_max_size(30)
        .with_first_dose(1)
        .build()
...
def test_efftox_docstring_example():
    ...
    trial = (
        ...
        .with_max_size(30)
        .with_first_dose(3)
        .with_kwargs(num_integral_steps=10**5)
        .build()
    )
    assert trial.next_dose() == 3
...
def test_efftox_initialization_with_skeleton():
    ...
    trial = (
        EffToxBuilder()
        .with_real_doses(real_doses)
        .with_cutoffs(0.4, 0.2)
        .with_certainties(0.8, 0.8)
        .with_metric(metric)
        .with_max_size(30)
        .with_kwargs(prior_tox_probs=prior_tox_probs, prior_eff_probs=prior_eff_probs)
        .build()
    )
