...
def test_hub_service_worker_inline_registration() -> None:
    ...
    assert "navigator.serviceWorker.register" in head_content, (
        "Service worker registration not found inside <head>."
    )
    ...
    assert script_match is not None, (
        "Failed to locate the inline script block for service worker registration."
    )
    ...
    assert "const " not in script_block, (
        "Found 'const' in inline script, violating ES5 legacy compatibility."
    )
    # No "let" variable declarations (use "var" instead)
    assert "let " not in script_block, (
        "Found 'let' in inline script, violating ES5 legacy compatibility."
    )
    # No ES6 arrow functions (use "function" instead)
    assert "=>" not in script_block, (
        "Found '=>' (arrow function) in inline script, violating ES5 legacy compatibility."
    )
    # No ES6 string/array includes() (use indexOf() !== -1 instead)
    assert ".includes(" not in script_block, (
        "Found '.includes(' in inline script, violating ES5 legacy compatibility."
    )

    # Verify that standard root scope and subpath scope are handled correctly
    assert "/clintrials/" in script_block, (
        "Subpath scope '/clintrials/' is not configured/handled in registration."
    )
    assert "indexOf" in script_block, (
        "indexOf should be used to check path for subpath."
    )
