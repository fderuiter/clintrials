...
def test_winratio_trial():  # type: ignore
    ...
    assert trial.has_more() is True  # type: ignore
    trial.update()  # type: ignore
    assert trial.has_more() is False  # type: ignore
    report = trial.report()  # type: ignore
    assert "success" in report
    assert "ci" in report
    trial.reset()  # type: ignore
