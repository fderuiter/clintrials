...
def test_watu_mc_config():
    ...
    trial2 = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric, 1, 64,
                  mc_sample_size=2000, mc_samples_stage1=3000, mc_samples_stage2=4000)
    assert trial2.mc_sample_size == 2000
    assert trial2.mc_samples_stage1 == 3000
    assert trial2.mc_samples_stage2 == 4000

    # Test clamping
    trial3 = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric, 1, 64,
                  mc_sample_size=500, mc_samples_stage1=600, mc_samples_stage2=700)
    ...
    trial4 = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric, 1, 64,
                  mc_sample_size=5000)
...
def test_watu_update_kwargs(mocker):
    ...
    trial = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric, 1, 64, stage_one_size=5)

    cases = [(1, 0, 1)]

    # Mock prob_tox_exceeds and prob_eff_exceeds to check n
    mock_crm_exceeds = mocker.patch.object(trial.crm, 'prob_tox_exceeds', return_value=np.array([0.1]))
    mock_eff_exceeds = mocker.patch.object(trial, 'prob_eff_exceeds', return_value=np.array([0.9]))
