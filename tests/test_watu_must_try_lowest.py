...
def test_watu_must_try_lowest_dose():
    ...
    trial_t = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric,
                 first_dose=3, max_size=30, stage_one_size=10, must_try_lowest_dose=True)
    assert trial_t.next_dose() == 1

    # Scenario 2: must_try_lowest_dose=False
    trial_f = WATU(skeletons, tox_prior, tox_target, tox_cutoff, eff_cutoff, metric,
                 first_dose=3, max_size=30, stage_one_size=10, must_try_lowest_dose=False)
...
def test_watu_reset_honors_flag():
    ...
    skeletons = [[0.1]*6]
    metric = LpNormCurve(0.05, 0.4, 0.25, 0.15)
    trial = WATU(skeletons, tox_prior, 0.30, 0.33, 0.05, metric,
                 first_dose=3, max_size=30, must_try_lowest_dose=True)
