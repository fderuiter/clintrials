...
def clean_ipython_magics(code_str):
...
def resolve_node(node, bindings):
...
class NotebookASTVisitor(ast.NodeVisitor):
    ...
    def __init__(self, manifest_elements, bindings=None):
        self.manifest_elements = manifest_elements
        self.bindings = bindings if bindings is not None else {}
        self.referenced = set()
        self.private_violations = []  # list of tuples (lineno, name, msg)
...
def load_public_elements(manifest_path):
...
def audit_single_notebook(notebook_path, manifest_elements):
...
def main():
    ...
    if not manifest_elements:
        print(f"Error: Loaded 0 public elements from '{manifest_path}'.")
        sys.exit(1)

    notebook_files = find_notebook_files(args.path)
    if not notebook_files:
        print(f"No notebooks found at path '{args.path}'.")
        sys.exit(0)
    ...
    print("\n" + "="*40)
    print("Notebook Standards and API Coverage Report")
    print("="*40)
    print(f"Total issues found: {total_issues_count}")
    print(f"Public API elements in manifest: {len(manifest_elements)}")
    print(f"Unique public API elements referenced: {len(covered_elements)}")
    print(f"Computed API Coverage: {coverage_pct:.2f}%")
    
    # Check coverage threshold
    coverage_failed = False
    if coverage_pct < args.min_coverage:
        print(f"  [FAIL] Coverage {coverage_pct:.2f}% is below the required minimum of {args.min_coverage:.2f}%")
        coverage_failed = True
    else:
        print(f"  [OK] Coverage matches or exceeds target of {args.min_coverage:.2f}%")

    print("="*40)

    if total_issues_count > 0 or coverage_failed:
        print("\nAudit failed. Fix the issues or raise coverage to pass.")
        sys.exit(1)
    else:
