...
def get_module_name(file_path):
    ...
    e.g., clintrials/core/recruitment.py -> recruitment
    """
...
def audit_commits(base_ref, head_ref, override_branch_name=None):
    ...
    if branch_name and branch_name.startswith('hotfix/'):
        print(f"Skipping TDD audit for hotfix branch: {branch_name}")
        return True
    ...
    if not commits:
        print("No commits found in PR range.")
        return True
    ...
    for commit in commits:
        ...
        if re.search(r'(?im)^skip-tdd\b', msg):
            print(f"Skipping TDD audit due to skip-tdd trailer in commit {commit}.")
            return True
        ...
        for f in files:
            ...
            if module_name:
                ...
                if not test_found:
                    print(f"TDD Audit Failed: Commit {commit} modifies source file '{f}' (module: '{module_name}') "
                          f"but no corresponding test file (e.g. 'tests/test_{module_name}.py') was added/modified "
                          "in this or any preceding commit.")
                    return False
                    
    print("TDD Audit passed.")
    return True

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: audit_timeline.py <base_ref> <head_ref>")
        sys.exit(1)
